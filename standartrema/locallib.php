 ...
 * @copyright   2022 standartrema - {@link https://standartrema.tech/}
 * @author      Rodrigo Mady
 ...
 */
function theme_standartrema_get_cards_settings() {
    $theme = theme_config::load('standartrema');
    $cardssettings = array();

    $numberofcards = get_config('theme_standartrema', 'numberofcards');
    if (get_config('theme_standartrema', 'frontpageenablecards') && $numberofcards > 1) {
 ...
 */
function theme_standartrema_get_disk_usage() {
    global $CFG;

    $cache = cache::make('theme_standartrema', 'dashboardadmin');
 ...
 */
function theme_standartrema_count_active_courses() {
    global $DB;
    $cache = cache::make('theme_standartrema', 'dashboardadmin');
 ...
 */
function theme_standartrema_count_courses() {
    global $DB;
    $cache = cache::make('theme_standartrema', 'dashboardadmin');
 ...
 * @return int number of active courses
 */
function theme_standartrema_get_active_courses() {
    global $DB;
    $cache = cache::make('theme_standartrema', 'dashboardadmin');
 ...
 * @return void
 */
function theme_standartrema_count_active_enrolments() {
    global $DB;
    $cache = cache::make('theme_standartrema', 'dashboardadmin');
    $activeenrolments = $cache->get('activeenrolments');
    if (!$activeenrolments) {
        $today = time();
        $activecourses = theme_standartrema_get_active_courses();
        if ($activecourses) {
 ...
 * @return void
 */
function theme_standartrema_count_users_enrolments() {
    global $DB;
    $cache = cache::make('theme_standartrema', 'dashboardadmin');
 ...
 * @return false|mixed
 */
function theme_standartrema_get_environment_issues() {
    global $CFG;
    $cache = cache::make('theme_standartrema', 'dashboardadmin');
 ...
 * @return void
 */
function theme_standartrema_setting_file_url($setting, $filearea, $theme) {
    global $CFG;

    $component  = 'theme_standartrema';
    $itemid     = 0;
 ...
 */
function theme_standartrema_page_init(moodle_page $page) {
    global $COURSE, $USER;
    ...
    if ($COURSE->id > 1) {
        ...
        if (file_exists($page->theme->dir.$coursestyle)) {
            $page->requires->css(new moodle_url("/theme/standartrema{$coursestyle}"));
        }
    }
}
